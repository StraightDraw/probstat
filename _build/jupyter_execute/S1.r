...
Native R does not have a single function that generates all the needed descriptive statistics, but Mosaic does.
...
When the mean and median of a data set differ</br>
by more than one-tenth of a standard deviation,</br>
we should expect skew and outliers. 

$$|\overline x - \text{med}| \geq s\hspace{3mm}\implies \hspace{3mm}\text{check skew}$$

Not official stats info. No one does this but me.</br>
Just a helpful way of considering if the the</br>
difference between mean and median is significant</br>
...
<p>We need to force RStudio to use the bin widths we want. The **width** parameter will force a bin width of our choosing, and the **center** parameter forces a single bin to be centered at that point. All  bins have equal widths so we don't have to specify the center of the histogram, just the center of one bin. We created a frequency table with a bin width of 20 above, and the center of each bin is an odd multiple of 10. </p>
...
R has dozens of built-in data frames like **iris**. Type variable name only and execute code block.</br>
Output shows headers, variable names and observations. Search the variable name in help tab of bottom-right panel for details.
